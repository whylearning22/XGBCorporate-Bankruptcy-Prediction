# -*- coding: utf-8 -*-
"""
Created on Mon Oct 27 17:17:39 2025
...
pipe_inner = ImbPipeline([
    ('smote', SMOTE(random_state=seed)),
    ('clf', dt)  
])
# Griglia iperparametri: solo max_depth
p_grid= {'clf__max_depth': [2, 3, 4, 5, 10, 15, 20, 30, 50]}


# -------------------------------
# Loop outer con SMOTE solo nel train esterno
# -------------------------------
metrics_per_fold = []
...
# -------------------------------
# DataFrame con metriche per fold
# -------------------------------
df_metrics = pd.DataFrame(metrics_per_fold)
df_metrics.to_csv("metriche_albero_SMOTE_outlier_taiwan.csv", index=False)
# Media e deviazione standard
metrics_mean = df_metrics.mean()
metrics_std = df_metrics.std()

# -------------------------------
# Confusion matrix globale (OOF)
# -------------------------------
cm = confusion_matrix(y, predictions)
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['Non-Bankrupt', 'Bankrupt'])
disp.plot(cmap='Blues')
plt.title("Matrice di Confusione - Nested CV (Decision Tree, SMOTE solo OUTER)")
plt.tight_layout()
plt.show()
...
plt.figure()
plt.plot(fpr, tpr, label=f'ROC (AUC globale = {roc_auc:.3f})')
plt.plot([0, 1], [0, 1], linestyle='--', color='gray')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Curva ROC - Nested CV (Decision Tree, SMOTE solo OUTER)')

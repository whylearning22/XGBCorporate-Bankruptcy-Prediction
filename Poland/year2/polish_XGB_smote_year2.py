# -*- coding: utf-8 -*-
"""
Created on Mon Oct 27 11:08:47 2025
...
# Pipeline: imputazione -> scaling -> SMOTE -> XGB
pipe = Pipeline([
...
# Grid Search sull’inner CV (solo training fold)
gs = GridSearchCV(
    estimator=pipe,
    param_grid=param_grid,
    cv=inner_cv,
    scoring='recall',    # ottimizza il recall 
    n_jobs=-1,
    refit=True
)
...
df_metrics.to_csv("metriche_xgboost_smote_year2_polish.csv", index=False)
...
plt.title("Matrice di Confusione - XGBoost (Year=2, SMOTE)")
plt.tight_layout()
plt.show()
...
plt.title('Curva ROC - XGBoost (Year=2, SMOTE)')
plt.legend(loc='lower right')
plt.tight_layout()
plt.show()

print("\nMedia metriche per fold:")
print(metrics_mean.round(3))
print("\nDeviazione standard metriche per fold:")
print(metrics_std.round(3))

# -*- coding: utf-8 -*-
"""
Created on Fri Oct 24 11:20:17 2025
...
for train_idx, test_idx in outer_cv.split(X, y):
    ...
    # --- Modello XGB con scale_pos_weight del training fold
    xgb = XGBClassifier(
        random_state=seed,
        n_jobs=-1,
        eval_metric='logloss',
    ...
    # --- Pipeline: imputer (fit su training) -> scaler (fit su training) -> xgb
    pipe = Pipeline([
    ...
    gs = GridSearchCV(
        ...
        cv=inner_cv,
        scoring='recall',   # metriche di selezione: qui massimizzi la recall
        n_jobs=-1,
        refit=True
    )
...
df_metrics.to_csv("metriche_xgboost_year4_polish.csv", index=False)
...
df_importances_top.to_csv("feature_importance_top20_xgboost_year4_polish.csv", index=False)
...
plt.ylabel("Feature")
plt.title(f"Top {top_n} XGBoost Feature Importances (Mean over 10 folds, Year=4)")
...
plt.title("Matrice di Confusione - XGBoost (Year=4)")
plt.tight_layout()
plt.show()
...
plt.title('Curva ROC - XGBoost (Year=4)')
